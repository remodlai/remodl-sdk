...
from remodl.types.router import LiteLLM_Params
...
class LiteLLMProxyChatConfig(OpenAIGPTConfig):
    ...
    def get_models(
        ...
        return [f"remodl_proxy/{model}" for model in models]
    ...
    @staticmethod
    def _should_use_remodl_proxy_by_default(
        remodl_params: Optional[LiteLLM_Params] = None,
    ):
        """
        Returns True if remodl proxy should be used by default for a given request

        Issue: https://github.com/BerriAI/remodl/issues/10559

        Use case:
        - When using Google ADK, users want a flag to dynamically enable sending the request to remodl proxy or not
        - Allow the model name to be passed in original format and still use remodl proxy:
        ...
        if get_secret_bool("USE_LITELLM_PROXY") is True:
            return True
        if remodl_params and remodl_params.use_remodl_proxy is True:
            return True
        if remodl.use_remodl_proxy is True:
            return True
        return False

    @staticmethod
    def remodl_proxy_get_custom_llm_provider_info(
        model: str, api_base: Optional[str] = None, api_key: Optional[str] = None
    ) -> Tuple[str, str, Optional[str], Optional[str]]:
        """
        Force use remodl proxy for all models

        Issue: https://github.com/BerriAI/remodl/issues/10559

        Expected behavior:
        - custom_llm_provider will be 'remodl_proxy'
        ...
        if model.startswith("remodl_proxy/"):
