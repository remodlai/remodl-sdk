...
class HealthCheckHelpers:
    ...
    async def ahealth_check_wildcard_models(
        ...
        from remodl import acompletion
        from remodl.remodl_core_utils.llm_request_utils import (
        ...
        if len(cheapest_models) == 0:
            raise Exception(
                f"Unable to health check wildcard model for provider {custom_llm_provider}. Add a model on your config.yaml or contribute here - https://github.com/BerriAI/remodl/blob/main/model_prices_and_context_window.json"
            )
    ...
    def _update_model_params_with_health_check_tracking_information(
        ...
        from remodl.proxy.remodl_pre_call_utils import LiteLLMProxyRequestSetup

        _metadata_variable_name = "remodl_metadata"
        remodl_metadata = HealthCheckHelpers._get_metadata_for_health_check_call()
        model_params[_metadata_variable_name] = remodl_metadata
        model_params = LiteLLMProxyRequestSetup.add_user_api_key_auth_to_request_metadata(
            data=model_params,
            user_api_key_dict=UserAPIKeyAuth.get_remodl_internal_health_check_user_api_key_auth(),
    ...
    def _get_metadata_for_health_check_call():
        ...
        """
        from remodl.constants import LITTELM_INTERNAL_HEALTH_SERVICE_ACCOUNT_NAME
    ...
    def get_mode_handlers(
        ...
        """
        import remodl
        from remodl.remodl_core_utils.audio_utils.utils import (
            get_audio_file_for_health_check,
        )
        from remodl.remodl_core_utils.health_check_utils import _filter_model_params
        from remodl.realtime_api.main import _realtime_health_check

        return {
            "chat": lambda: remodl.acompletion(
                **model_params,
            ),
            "completion": lambda: remodl.atext_completion(
                **_filter_model_params(model_params=model_params),
                prompt=prompt or "test",
            ),
            "embedding": lambda: remodl.aembedding(
                **_filter_model_params(model_params=model_params),
                input=input or ["test"],
            ),
            "audio_speech": lambda: remodl.aspeech(
                **{
                    **_filter_model_params(model_params=model_params),
                    **(
                        {"voice": "alloy"}
            ...
            "audio_transcription": lambda: remodl.atranscription(
                **_filter_model_params(model_params=model_params),
                file=get_audio_file_for_health_check(),
            ),
            "image_generation": lambda: remodl.aimage_generation(
                **_filter_model_params(model_params=model_params),
                prompt=prompt,
            ),
            "rerank": lambda: remodl.arerank(
                **_filter_model_params(model_params=model_params),
                query=prompt or "",
            ...
            "batch": lambda: remodl.alist_batches(
                **_filter_model_params(model_params=model_params),
            ),
            "responses": lambda: remodl.aresponses(
                **_filter_model_params(model_params=model_params),
                input=prompt or "test",
            ),
            "ocr": lambda: remodl.aocr(
                **_filter_model_params(model_params=model_params),
                document={
                    "type": "document_url",
